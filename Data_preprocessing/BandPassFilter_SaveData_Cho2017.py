...
##======Cho2017 Dataset========
def process_raw(raw, dataset, channels=None, baseline=None, resample=None):
    ...
    Params:
        ...
        resample: float, default: None, If not None, resample the eeg data with the sampling rate provided.
    Return:
    ...
    # set the labels as class 0 (left) and class 1 (right) data
    labels = labels-1

    return x, labels


def Cho2017_fetchfiles(saveNpzPath, chans=None, downsampleFactor=1):

    # download the data
    data_orig = Cho2017()
    subjects_list = list(range(1, 53))
    # Update on 2023.5.25, Subject 32,46,49's data have been found on GigaDB
    # for ii in [32, 46, 49]:
    #     subjects_list.remove(ii)
    # add 32, 46, 49 to the subjects_list
    for ii in [32, 46, 49]:
        data_orig.subject_list.append(ii)

    # for sub_idx in subjects_list:
    #     path_name = data_orig.data_path(sub_idx)
    #     print(path_name)
    ...
    if downsampleFactor == 1:
        ...
        resample = 512/downsampleFactor
        fs = 512/downsampleFactor
    ...
    for sub_idx in subjects_list:
        ...
        for session, runs in sessions.items():
            for run, raw in runs.items():
                x, y = process_raw(raw, data_orig, channels=chans, resample=resample)
                met = {}
                met["subject"] = sub_idx
                met["session"] = session
                met["run"] = run
...
def preprocessing_Cho2017_dataset(saveNpzPath, saveFilteredPath, chans, downdampleFactor, bandFiltCutF, filterType, resample):

    # check if the original file exists or fetch it.
    if not os.path.exists(saveNpzPath):
        os.makedirs(saveNpzPath)
    Cho2017_fetchfiles(saveNpzPath, chans=chans, downsampleFactor=downdampleFactor)
...
if __name__ == '__main__':
    ...
    saveNpzPath = r'/path/to/Original_data/Cho2017'
    saveFilteredPath = r'/path/to/filtered_data/Cho2017'
    ...
    preprocessing_Cho2017_dataset(saveNpzPath, saveFilteredPath, chans, downdampleFactor, bandFiltCutF, filterType, resample)