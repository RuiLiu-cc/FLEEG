...
def shin2017a_fetchfiles(saveNpzPath, chans=None, downsampleFactor=1):

    # download the data
    data_orig = Shin2017A(accept=True)

    # get the EEG
    data_all = data_orig.get_data()

    if downsampleFactor == 1:
        resample = None
    else:
        resample = 200/downsampleFactor

    for subject, sessions in data_all.items():
        metadata = []
        ...
        filename = os.path.join(saveNpzPath, 's{}.npz'.format(subject))
        np.savez(filename, x_data=x_data, y_data=y_data, metadata=metadata, fs=200)
...
def preprocessing_shin2017a_dataset(saveNpzPath, saveFilteredPath, chans, downdampleFactor, bandFiltCutF, filterType, resample):

    # check if the original file exists or fetch it.
    if not os.path.exists(saveNpzPath):
        os.makedirs(saveNpzPath)
        shin2017a_fetchfiles(saveNpzPath, chans=chans, downsampleFactor=downdampleFactor)
    ...
    if not os.path.exists(FilteredPath):
        ...
        for filename in files:
            ...
            # resample the EEG signal from 200Hz to 250Hz
            if resample:
                print('Resampling...')
...
if __name__ == '__main__':
    ...
    saveNpzPath = r'/path/to/Original_data/Shin_MI/One_file_one_sub'
    saveFilteredPath = r'/path/to/filtered_data/Shin_MI/'
    ...
    preprocessing_shin2017a_dataset(saveNpzPath, saveFilteredPath, chans, downdampleFactor, bandFiltCutF, filterType, resample)