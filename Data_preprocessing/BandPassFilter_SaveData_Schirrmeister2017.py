...
def Schirrmeister2017_fetchfiles(saveNpzPath, chans=None, downsampleFactor=1):

    # download the data
    data_orig = Schirrmeister2017()
    for sub_idx in range(4,15):
    ...
    if downsampleFactor == 1:
        ...
        resample = 500/downsampleFactor
        fs = 500/downsampleFactor

    # get the EEG one by one
    for sub_idx in range(1,15):
...
def preprocessing_Schirrmeister2017_dataset(saveNpzPath, saveFilteredPath, chans, downdampleFactor, bandFiltCutF, filterType, resample):

    # check if the original file exists or fetch it.
    if not os.path.exists(saveNpzPath):
        os.makedirs(saveNpzPath)
    Schirrmeister2017_fetchfiles(saveNpzPath, chans=chans, downsampleFactor=downdampleFactor)
    ...
    if not os.path.exists(FilteredPath):
        ...
        for filename in files:
            ...
            y_data = y_data - 1
            x = x[np.all([y_data < 2], axis=0), :, :]
            y_data = y_data[y_data < 2]
            # exchange the labels from right=0&left=1 to right=1&left=0
            y_data = 1-y_data
...
if __name__ == '__main__':
    ...
    saveNpzPath = r'/path/to/Original_data/Schirrmeister2017'
    saveFilteredPath = r'/path/to/filtered_data/Schirrmeister2017'
    ...
    preprocessing_Schirrmeister2017_dataset(saveNpzPath, saveFilteredPath, chans, downdampleFactor, bandFiltCutF, filterType, resample)